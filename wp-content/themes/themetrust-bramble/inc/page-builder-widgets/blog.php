...
/**
* Class to bramble a custom widget for displaying blog
*/
...
class TTrust_Blog extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 'classname' => 'bramble-blog', 'description' => __('Display blog.', 'bramble' ) );

		// Instantiate parent
		parent::__construct(

			'ttrust_blog', // Base ID
			__( 'Bramble Blog', 'bramble' ), // Name
			 $widget_ops // Args
	...
	public function widget( $args, $instance ) {
			...
			$boxed_class = ($instance['boxed'] == "yes") ? 'boxed' : '';
			...
			?>
				<div id="<?php echo $args['widget_id']; ?>" class="blog clear <?php echo $layout_class; ?> <?php echo $column_class; ?> <?php echo $boxed_class; ?> <?php echo $alignment_class; ?>">
...
<?php
	...
	function form( $instance ) {
		...
		$boxed = (isset($instance['boxed'])) ? esc_attr($instance['boxed']) : '';
		...
		<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e( 'Number of Posts', 'bramble' ); ?></label>
		<input type="number" name="<?php echo $this->get_field_name( 'count' ); ?>" min="1" max="99" value="<?php echo $count; ?>">

		<p>
		<label for="<?php echo $this->get_field_id('category'); ?>"><?php _e('Category (leave blank to show all)', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('alignment'); ?>"><?php _e('Content Alignment', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('boxed'); ?>"><?php _e('Boxed', 'bramble'); ?></label>
		<select name="<?php echo $this->get_field_name('boxed'); ?>" id="<?php echo $this->get_field_id('boxed'); ?>" class="widefat">
		<?php
		$options = array('Yes'=>'yes', 'No'=>'no');
		foreach ($options as $option => $value) {
		$selected = ($boxed == $value) ? ' selected="selected"' : '';
		echo '<option value="' . $value . '" id="' . $value . '"' . $selected . '>'. $option . '</option>';
		}
		?>
		</select>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('show_excerpt'); ?>"><?php _e( 'Show Excerpt', 'bramble' ); ?></label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'carousel-nav-color' ); ?>" style="display:block;"><?php _e( 'Carousel Nav Color', 'bramble' ); ?></label> 
