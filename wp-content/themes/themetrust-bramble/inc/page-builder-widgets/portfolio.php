...
/**
* Class portfolio to bramble a custom widget for displaying projects
*/
...
class TTrust_Portfolio extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 'classname' => 'bramble-portfolio', 'description' => __('Display portfolio projects.', 'bramble' ) );

		// Instantiate parent
		parent::__construct(

			'ttrust_portfolio', // Base ID
			__( 'Bramble Portfolio', 'bramble' ), // Name
			 $widget_ops // Args
	...
	public function widget( $args, $instance ) {
			...
			$filter_selected_color = $instance['filter_selected_color'];
			$skills = array();
				...
				<div class="projects <?php echo $layout_class; ?> <?php echo $column_class; ?>">
					...
					<?php
						// FilterNav output
							...
							if( sizeof( $skills ) > 0 ) { 
								if($show_filter=="yes"){?>
								<ul id="filter-nav" class="clearfix <?php echo $filter_alignment; ?>" style="border-color: <?php echo $instance['filter_selected_color']; ?>;">
									<li class="all-btn"><a data-filter="*" class="selected"><?php _e( 'All', 'bramble' ); ?></a></li>
									<?php
									foreach( $skills as $skill ) {
										$skill = get_term_by( 'slug', trim(htmlentities($skill)), 'skill');
										if($skill) {
										$output = sprintf( '<li><a data-filter=".%1$s">%2$s</a></li>%3$s',
												esc_attr( $skill->slug ),
							...
							<?php
								...
								if($show_filter=="yes"){?>
								<ul id="filter-nav" class="clearfix <?php echo $filter_alignment; ?>" style="border-color: <?php echo $instance['filter_selected_color']; ?>;">
									<li class="all-btn"><a data-filter="*" class="selected"><?php _e('All', 'themetrust'); ?></a></li>
									<?php $j=1;
									$skills = get_terms('skill');
									foreach ($skills as $skill) {
										$a = '<li><a data-filter=".'.$skill->slug.'">';
								    	$a .= $skill->name;					
										$a .= '</a></li>';
										echo $a;
										echo "\n";
					...
					<div class="load-more-holder">
						...
						<div class="loading hidden button" style="background:<?php echo $load_more_color; ?>; color:<?php echo $load_more_text_color; ?>;"><?php _e( "Loading", 'bramble'); ?></div>
					</div>
					<?php } ?>
				</div><!-- .projects -->
...
<?php
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['filter_selected_color'] = $new_instance['filter_selected_color'];
	...
	function form( $instance ) {
		...
		$filter_selected_color = (isset($instance['filter_selected_color'])) ? esc_attr($instance['filter_selected_color']) : '';
		...
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Name', 'bramble' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $title; ?>" /></p>
		
		<p>
		    
			<label for="<?php echo $this->get_field_id('show_filter'); ?>"><?php _e( 'Show Filter', 'bramble' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'filter_selected_color' ); ?>" style="display:block;"><?php _e( 'Filter Selected Color', 'bramble' ); ?></label> 
			<input class="widefat color-picker" id="<?php echo $this->get_field_id( 'filter_selected_color' ); ?>" name="<?php echo $this->get_field_name( 'filter_selected_color' ); ?>" type="text" value="<?php echo esc_attr( $hover_color ); ?>" />
		</p>
		
		<p>
		<label for="<?php echo $this->get_field_id('filter_alignment'); ?>"><?php _e('Filter Alignment', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e( 'Number of Projects', 'bramble' ); ?></label>
		<input type="number" name="<?php echo $this->get_field_name( 'count' ); ?>" min="1" max="99" value="<?php echo $count; ?>">
		</p>
		
		<p>
		<label for="<?php echo $this->get_field_id('thumb_proportions'); ?>"><?php _e('Featured Image Proportions for Non-masonry Layout ', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('layout'); ?>"><?php _e('Layout', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('columns'); ?>"><?php _e('Columns', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('skills'); ?>"><?php _e('Show only certain skills', 'bramble'); ?></label>
		<?php
		...
		<p>
		<label for="<?php echo $this->get_field_id('orderby'); ?>"><?php _e('Order By', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('order'); ?>"><?php _e('Order', 'bramble'); ?></label>
		...
		<p>
		<label for="<?php echo $this->get_field_id('hover_effect'); ?>"><?php _e('Hover Effect', 'bramble'); ?></label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'hover_color' ); ?>" style="display:block;"><?php _e( 'Hover Color', 'bramble' ); ?></label> 
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'hover_text_color' ); ?>" style="display:block;"><?php _e( 'Hover Text Color', 'bramble' ); ?></label> 
		...
		<p>
		<label for="<?php echo $this->get_field_id('show_skills'); ?>"><?php _e('Show Skills on Hover', 'bramble'); ?></label>
		...
		<p>		    
			<label for="<?php echo $this->get_field_id('show_load_more'); ?>"><?php _e( 'Show Load More Button', 'bramble' ); ?></label>
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'load_more_color' ); ?>" style="display:block;"><?php _e( 'Load More Button Color', 'bramble' ); ?></label> 
		...
		<p>
		    <label for="<?php echo $this->get_field_id( 'load_more_text_color' ); ?>" style="display:block;"><?php _e( 'Load More Button Text Color', 'bramble' ); ?></label> 
		...
		<p>		    
			<label for="<?php echo $this->get_field_id('enable_lightbox'); ?>"><?php _e( 'Enable Lightbox', 'bramble' ); ?></label>
