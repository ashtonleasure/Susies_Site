 ...
 */
add_filter ( 'woocommerce_add_to_cart_redirect', 'bramble_wc_redirect_to_checkout' );

function bramble_wc_redirect_to_checkout() {
	global $woocommerce;
...
/**
 * Brambles a buy-now button
 *
 * @param str $product_id   The ID of the product to buy
 *
 * @return string $output   HTML for the Buy Now button
 */
function bramble_wc_the_buy_now( $product_id ) {

	$buy_now = do_shortcode( '[add_to_cart_url id="' . $product_id . '"]' );

	$output = '<a href="' . $buy_now . '&buy_now=true" class="buy-now">' . __( 'Buy Now', 'bramble' ) . '</a>';

	echo $output;

} // bramble_wc_the_buy_now


/**
 * Define image sizes
 */
function bramble_wc_image_dimensions() {
	global $pagenow;
...
add_action( 'after_switch_theme', 'bramble_wc_image_dimensions', 1 );
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	ob_start();
	?>
	<a class="cart-icon cart-contents right open" href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'bramble'); ?>">
...
$products_per_page = get_theme_mod( 'bramble_shop_product_count', 12 );
add_filter( 'loop_shop_per_page', function($products_per_page){ return $products_per_page; }, 20 );
