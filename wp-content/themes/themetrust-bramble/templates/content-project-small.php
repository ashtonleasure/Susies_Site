...
$skills = bramble_get_skills( $post ); /** @see inc/template-tags.php */
...
if($portfolio_config['masonry']){ //check if masonry layout is enabled
	$size = get_post_meta( $id, '_bramble_project_featured_image_size', true );
	$thumb_size = 'bramble_thumb_' . $size;
	$size_class = 'masonry-'.$size;
}else{
	$thumb_size = 'bramble_thumb_' . $portfolio_config['thumb_proportions'];
}
$lightbox_video = "";
$lightbox_img = "";
if($portfolio_config['enable_lightbox'] == "yes"){ //check if lightbox mode is enabled
	$lightbox_video = get_post_meta( $id, '_bramble_project_lightbox_video', true );
	$lightbox_img = get_post_meta( $id, '_bramble_project_lightbox_img', true );
}
?>
						...
						<div class="inside">
							...
							<div class="details <?php echo $details_class; ?>" style="border-color: <?php echo $portfolio_config['hover_text_color']; ?>;">
								...
								<div class="skills" style="color: <?php echo $portfolio_config['hover_text_color']; ?>;">
									<?php bramble_the_thumb_skills(); /** @see inc/template-tags.php */ ?>
								</div>
								<?php } ?>
